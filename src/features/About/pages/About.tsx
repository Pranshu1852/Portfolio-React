      ...
      <div>
        ...
        <div className="ml-11">
          {educationData.map((item, index) => (
            <TimeLine key={index} title={item.title} period={item.period} isLast={index===educationData.length-1} />
          ))}
        </div>
      </div>
      <div>
        ...
        <div className="ml-11">
          {experienceData.map((item, index) => (
            <TimeLine key={index} title={item.title} period={item.period} isLast={index===experienceData.length-1} />
          ))}
        </div>
      </div>
        ...
        <div className="relative bg-gradient-to-br from-[#404040] to-transparent p-6 rounded-2xl shadow-lg">
          ...
          <div className='flex flex-col gap-5 relative'>
            {skillsData.map((item, index) => (
              <SkillBar key={index} name={item.name} percentage={item.percentage} />
            ))}
          </div>
        </div>
      </div>
